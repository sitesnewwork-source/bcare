...
const VisitorDetailsPanel: React.FC<Props> = ({
  ...
  return (
    ...
    >
      ...
      {/* Compact Header */}
      <div className="bg-gradient-to-l from-primary/5 to-transparent border-b border-border/60 p-4 shrink-0">
        <div className="flex items-start gap-3">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-md bg-gradient-to-br ${avatarColor.bg} border border-white/20`}>
            <span className={`text-lg font-bold ${avatarColor.text}`}>{initial}</span>
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-base font-bold text-foreground truncate">{selectedVisitor.visitor_name || "زائر"}</h2>
              ...
              <button onClick={() => onToggleFavorite(selectedVisitor.id)} className="p-0.5">
                <Star className={`w-4 h-4 ${selectedVisitor.is_favorite ? "fill-amber-400 text-amber-400" : "text-muted-foreground hover:text-amber-400"} transition-colors`} />
              </button>
            </div>
            <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground/70 flex-wrap">
              <span className="inline-flex items-center gap-1"><MapPin className="w-3 h-3 text-primary" />{selectedVisitor.current_page || "/"}</span>
              ...
              <span className="inline-flex items-center gap-1"><Clock className="w-3 h-3" />{getSessionDuration(selectedVisitor.created_at, selectedVisitor.last_seen_at)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 mt-3 flex-wrap">
          <button onClick={onBlockToggle} disabled={loadingAction === "block"} className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-bold border transition-all ${selectedVisitor.is_blocked ? "bg-destructive/10 text-destructive border-destructive/30" : "bg-muted/40 text-muted-foreground border-transparent hover:bg-muted"}`}>
            {selectedVisitor.is_blocked ? <><ShieldCheck className="w-3 h-3" />إلغاء الحظر</> : <><Ban className="w-3 h-3" />حظر</>}
          </button>
          <button onClick={onExportPDF} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-bold bg-muted/40 text-muted-foreground border border-transparent hover:bg-muted transition-all">
            <Download className="w-3 h-3" />تصدير PDF
          </button>
          <div className="flex items-center gap-1.5 flex-1 min-w-[180px]">
            <select value={redirectPage} onChange={e => setRedirectPage(e.target.value)} className="flex-1 h-7 rounded-lg border border-border bg-card px-2 text-[10px] text-foreground focus:outline-none focus:border-primary transition-all">
            ...
            <button onClick={() => onRedirect(redirectPage)} disabled={!redirectPage} className="h-7 px-3 rounded-lg text-[10px] font-bold bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-40 transition-all whitespace-nowrap">
              توجيه
            </button>
          </div>
        </div>
